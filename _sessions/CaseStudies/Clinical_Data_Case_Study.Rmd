...
2. Get a first impression of the objects you just created by exploring them with a mixture of the `View()`, `head()`, `names()`, and `summary()` functions. Were they all loaded correctly?
...
1. Was there a correlation between change in quality of life and histamine change? Answer this by creating the appropriate summary statistics, visualisation, and hypothesis test.
...
2. Did older patients tend to have higher histamine changes than younger patients? Answer this by creating the appropriate summary statistics, visualisation, and hypothesis test.

3. The variable `diseasestatus` indicates each patient's disease status at the beginning of the trial, which can be either "Early", "Mid" or "Late". Did patients in a Late disease state tend to respond better to dimarta compared to patients in Early disease state? Answer this by creating the appropriate summary statistics, visualisation, and hypothesis test.
