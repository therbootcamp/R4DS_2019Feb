...
```{r, echo = TRUE, eval = TRUE}
# read new data sets
appointments <- read_csv('https://therbootcamp.github.io/R4DS_2019Feb/1_Data/appointments.csv')

weather <- read_csv('https://therbootcamp.github.io/R4DS_2019Feb/1_Data/weather.csv')
```

2. Your goal is to use these datasets to answer the question: "Are patients more likely to not show up for their appointments during bad versus good weather conditions?" To do this, consider the following steps
...
- Think about which columns in each dataset are necessary to answer your question (e.g.; how do you define bad versus good weather? Which variable(s) indicate whether a patient showed up for their appointment or not?).
- Using `mutate()` and `case_when()`, add a new column to the `weather` data called `daytype` which indicates whether or not a day has `"good"`, `"bad"` or `"ok"` weather.
- Combine (aka, join) the two datasets into a dataset called `combined` based on a key column using `left_join()`
- Using `filter()`, create a dataset called `bad_days` containing only data for bad days.
- Using `filter()`, create a dataset called `good_days` containing only data for good days.
- For the `good_days` data, calculate the perentage of patients who did not show up for their appointment.
- For the `bad_days` data, calculate the same percentage.
...
appointments <- read_csv("https://github.com/therbootcamp/R4DS_2019Feb/blob/master/1_Data/appointments.csv?raw=true")

weather <- read_csv("https://github.com/therbootcamp/R4DS_2019Feb/blob/master/1_Data/weather.csv?raw=true")
