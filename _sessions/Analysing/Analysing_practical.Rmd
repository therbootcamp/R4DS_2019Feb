...
2. Open a new R script. At the top of the script, using comments, write your name and the date and "Analysing Practical". 
...
3. Save it as a new file called `analysing_practical.R` in the `2_Code` folder. 

4. Using `library()` load the `tidyverse` and `broom` packages for this practical listed in the Functions section above. If you don't have them installed, you'll need to install them, see the Functions tab above for installation instructions.
...
4. Using `print()`, `dim()`, `summary()`, `head()`, and `View()`, explore the data to make sure it was loaded correctly. The first few rows and columns should look like this:
...
*First 5 rows and 5 columns of kc_house*
...
3. Add new column(s) `living_sqm`, `above_sqm` and `basement_sqm` which show  the respective room sizes in square meters rather than square feet (Hint: Multiply each by 0.093).
...
4. Add a new column `total_sqm` which is the sum of `living_sqm`, `above_sqm` and `basement_sqm`
...
5. Add a new variable to the dataframe called `mansion` which is "Yes" when `total_sqm` is greater than 750, and "No" when `total_sqm` is less than or equal to 750.
...
6. How many mansions are there in the data and how many non mansions are there? Answer this by running `table(kc_house$mansion)`. You should find `r sum(kc_house$mansion == "Yes")` mansions and `r sum(kc_house$mansion == "No")` non-mansions!
...
```{r, eval = FALSE, echo = TRUE}
# Analysing with dplyr and tidyr ---------------------------
...
See the full [dplyr vignette](https://cran.r-project.org/web/packages/dplyr/vignettes/dplyr.html) with lots of wrangling tips and tricks.
